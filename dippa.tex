...
%%   linebreaks.
\thesistitle{SAM2 pseudolabeling for instance segmentation in UAV-based aerial imagery}
\place{Espoo}
...
\begin{enumerate}
\item How much does SAM2 pseudolabeling improve low quality instance segmentation annotations in UAV imagery?
\item How much does SAM2 pseudolabeling improve training results in instance segmentaion tasks in UAV imagery?
\item How does the addition of IR and NIR channels improve training on SAM2 pseudolabels?
...
A key feature of transformers is self-attention, which allows them to model interactions between task-relevant tokens in the input token sequence, even when they occur far apart.
\cite{transformer-survey}
...
The main dataset consists of a multispectral orthophoto taken from Espoonlahti by helicopter spanning approximately 2 km$^2$ and CHM-based coarse tree crown segments. The entire area is shown in Figure \ref{fig:dataset-ortho} and a magnified view of the test area is shown in Figure \ref{fig:dataset-test}). The orthophoto covers both forest and urban area, has a GSD of 0.05 m, has a width of 36871 pixels and a height of 31254 pixels, and contains blue, green, red, near-infrared, mid-infrared, and thermal infrared bands. The test area was cut out of the orthophoto and selected based on image quality and environment variety, and has a width of 4893 pixels and a height of 4471 pixels.
...
For inference the mask threshold was set to 0.5, the score/confidence threshold to 0.3, and for NMS, the IoU threshold was set to 0.3, as very little overlap is expected between tree individuals. Table \ref{table:inference} contains the inference parameters.
