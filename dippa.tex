...
The transformer is a more recent neural network architecture that has shown great performance when trained on extremely large amounts of data. The network consists of an encoder that takes as input a set of tokens, that are then weighed using a self-attention layer, after which a decoder calculates the output based on the output of the encoder \cite{attention-is-all-you-need}. The main advantage of transformers is the ability to leverage massively parallel compute efficiently, allowing the training of very large models in a reasonable amount of time. An example of a transformer type neural network in shown in Figure \ref{fig:transformer}.
...
Then, a Mask R-CNN model with a ResNet-50 \cite{resnet} backbone was trained separately on both the coarse segments and pseudolabels. The optimizer was AdamW \cite{adamw} with a learning rate of 1e-6 and weight decay of 1e-2. Training hyperparameters are listed in Table \ref{table:train}.
...
\begin{table}[h]
    ...
    Batch size & 8 \\
    Epochs & 400 \\
    Augmentations & Horizontal flipping, vertical flipping, rotation, random IoU crop \\
    \end{tabular}
...
Figure \ref{fig:loss} shows the training and validation loss over the training epochs.
...
Table \ref{table:inference} contains the inference parameters.
